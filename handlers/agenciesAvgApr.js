...
exports.handler = async ({ pathParameters, queryStringParameters }) => {
    try {
        ...
        let delegationContract = new SmartContract({ address: new Address('erd1qqqqqqqqqqqqqqqpqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqylllslmq6y6') });
        ...
        for (const provider of data.returnData) {
          address = Address.fromHex(Buffer.from(provider, 'base64').toString('hex'))
          contract = new SmartContract({address: address});
          ...
          owner_address = Address.fromHex(Buffer.from(contractConfig.returnData[0], 'base64').toString('hex'))
        ...
        return response({ status, agencies, fields });
    } catch (error) {
